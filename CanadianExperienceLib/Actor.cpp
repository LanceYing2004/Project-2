...
/**
 * iterate through every drawable, display dialog box only in MachineDrawable
 */
void Actor::DisplayDialogBox()
{
    for(auto drawable : mDrawablesInOrder)
    {
        drawable->DisplayDialogBox();
    }
}

/**
 * iterate through every drawable, set up Dialog box in MachineDrawable
 * @param window wxWindow
 */
void Actor::AccessDialogBox(wxWindow *window)
{
    for(auto drawable : mDrawablesInOrder)
    {
        drawable->AccessDialogBox(window);
    }
}
